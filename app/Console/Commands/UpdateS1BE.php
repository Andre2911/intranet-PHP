...
class UpdateS1BE extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'update:S1BE';
    ...
    public function handle()
    {
        ...
        for($i = 1; $i <= $mes; $i++ ){
            ...
            $url = 'http://172.28.206.57/apisybase_a/index.php/Stat/listarFormSIJ?form=S1B-E&n_anio='.$anio.'&n_mes='.$i;
            $consulta = $this->consultaAPI($url);
            Storage :: append("archivo.txt", 'actualización de DATA HITOS [S1B-E]: [AÑO]->'.$anio.'[MES]->'.$i.'[status]'.$consulta['status']);
            if($consulta['status'] == 200 && isset($consulta['data'])){
                Log::info('Registrando datos  S1B-E de DATA HITOS: [AÑO]->'.$anio.'[MES]->'.$i.'[#Resultados]->'.$consulta['data']);
                Log::info('Actualización de DATA HITOS: [AÑO]->'.$anio.'[MES]->'.$i.' [COMPLETO]');
            } else{
                Log::error('Error actualización de DATA HITOS S1B-E: [AÑO]->'.$anio.'[MES]->'.$i);
            }
        }
