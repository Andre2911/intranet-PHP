<template>
    <v-row >
        <v-layout wrap class="ma-2">
            <v-col cols="12" md="12" class="pt-3 pr-2 pl-2 pb-0">
                            ...
                            <tbody>
                                <tr v-for="item in items" :key="item.id">
                                    <td v-for="header in headers" :key="header.value">
                                        ...
                                        <template v-else-if="header.type=='concat' && item[header.value] != undefined">
                                            <template v-for="dato in header.subvalue_concat.split(',')">
                                                <template v-if="item[dato] != null">
                                                    {{item[dato]}}
                                                </template>
                                            </template>
                                        </template>
                                        ...
                                        <template v-else-if="header.type == 'archivo'  && item[header.filename] != undefined">
                                               
                                            <v-btn target="_blank" :href="'../'+header.ruta+item[header.filename]" fab small dark color="primary">
                                                <v-icon>
                                                    {{header.icon}}
                                                </v-icon>
                                            </v-btn>
...
<script>
export default {
    props:['headers', 'data', 'filters', 'search', 'n_paginas', 'orden', 'itemsPerPage', 'tipo', 'dense','pagina_selected', 'dosearch', 'allData'],
    data() {
        return {
            isMobile:false,
            pagina_actual:1,
